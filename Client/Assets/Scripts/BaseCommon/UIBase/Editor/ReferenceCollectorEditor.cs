...
public class ReferenceCollectorEditor: Editor
{
	...
	public override void OnInspectorGUI()
	{
        ...
        referenceCollector.viewType = (ViewType)EditorGUILayout.
        EnumPopup("脚本类型:", referenceCollector.viewType);
        ...
        for (int i = referenceCollector.data.Count - 1; i >= 0; i--)
		{
            ...
            ReferenceCollectorData data = referenceCollector.data[i];
            ...
            var obj = EditorGUILayout.ObjectField(property.objectReferenceValue, typeof(Object), true);

            #region 类型选择
            var objGO = obj as GameObject;
            var objCom = obj as Component;

            if (objGO || objCom)
            {
                var coms = objGO ? objGO.GetComponents(typeof(Component)) : objCom.GetComponents(typeof(Component));
                ...
                foreach (var item in coms)
                {
                    if (item==null)
                    {
                        LOG.logError("No exist error MonoBehaviour");
                        continue;
                    }
                    if (item.GetType() != typeof(ReferenceCollector))
                    {
                        comList.Add(item);
                    }
                }
                coms = comList.ToArray();
                ...
                {
                    names[j + 1] = coms[j].GetType().Name;
                }

                data.maskindex = EditorGUILayout.Popup("", data.maskindex, names, GUILayout.Width(80));

                if (data.maskindex == 0)
                    data.gameObject = obj is GameObject ? obj as GameObject : (obj as Component).gameObject;
                else if(data.maskindex - 1 < coms.Length)
                    data.gameObject = coms[data.maskindex - 1];
            }
            #endregion
