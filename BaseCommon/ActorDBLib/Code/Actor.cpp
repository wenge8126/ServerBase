...
namespace NetCloud
{
	...
	AComponent Actor::AddComponent(const AString &compName)
	{
	...
	bool Actor::RemoveComponent(const AString &compName)
	{
		AComponent existComp = mComponentList.find(compName);
		if (existComp)
		{
	...
	AutoNice Actor::Await(Auto<TransferPacket> tranPak, UnitID targetID, int overMilSecond)
	{
	...
	bool Actor::SendMsg(const AString &msgName, tBaseMsg &msg, UnitID targetID, BROADCAST_MODE eMode /*= eBroadcastNone*/)
	{
		AsyncNode *pNetNode = GetNetNode();
		if (pNetNode == NULL)
		{
	...
	bool Actor::OnReceiveProcess(NodePacket *pNodePacket)
	{
	...
	Logic::tEventCenter* Actor::GetEventCenter()
	{
		return GetMgr()->mEventCenter.getPtr();
	}

	bool Actor::WaitDestory(Logic::tEvent *pEvent)
	{
