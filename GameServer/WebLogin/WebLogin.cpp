...
int main()
{
	...
	if (WebLoginThread::IsAccountWeb())
	{
	...
	{
		SendMessage(hwnd, WM_SETICON, ICON_BIG, (LPARAM)LoadIcon(GetModuleHandle(0), MAKEINTRESOURCE(IDI_ICON_WSS_LOGIN)));
		SendMessage(hwnd, WM_SETICON, ICON_SMALL, (LPARAM)LoadIcon(GetModuleHandle(0), MAKEINTRESOURCE(IDI_ICON_WSS_LOGIN)));
	}
	...
	GetLocalTime(&nowtime);
			...
			INIT_RUN_CONFIG(LoginConfig, "login_config")

	logFileName.Format("./Log/%s_%4d-%02d-%02d_%02d-%02d-%02d.log", WebLoginThread::GetAppName(),
		nowtime.wYear, nowtime.wMonth, nowtime.wDay, nowtime.wHour, nowtime.wMinute, nowtime.wSecond);

	TableTool::SetLog(new TableLog(logFileName.c_str()));

	logErrorFileName.Format("./Error/%s_Error_%4d-%02d-%02d_%02d-%02d-%02d.log", WebLoginThread::GetAppName(),
		nowtime.wYear, nowtime.wMonth, nowtime.wDay, nowtime.wHour, nowtime.wMinute, nowtime.wSecond);

	Allot::setLogFile(WebLoginThread::GetAppName());
	...
	WebLoginThread *gpThread = MEM_NEW WebLoginThread();
	mServer->Start(gpThread, WebLoginThread::GetAppName(), logFileName.c_str(), NULL);
