#ifndef _INCLUDE_EXPORTBASE_H_
#define _INCLUDE_EXPORTBASE_H_
...
class CommonLib_Export_H ExportBase
{
public:
	static FieldInfo GetFieldInfoByTypeString(AString typeString)
	{
	...
	static AString splitTemplateType(AString type, bool bTs, bool *bIsBaseType = NULL)
	{
	...
	static AString GetNoteString(AString &scrString)
	{
	...
	static AString GetMemberNote(AutoNice noteData, const AString &memberName)
	{
	...
	static void ClearSpaceString(AString &infoStr)
	{
	...
	static AString GetTsType(AString typeString, AutoNice proList, bool bSetDefault)
	{
		AString tempType = typeString;
		AString leftType = tempType.SplitLeft(">");
		FieldInfo info = NULL;
	...
	static AString GenerateMsgProtocolCppCode(AutoNice proList, AutoNice proNotes, Array<AString> &structNameList, EasyHash<AString, bool> &exportRunHash, bool bFirst, bool bRunConfig, bool bIncludeMsgStruct)
	{
	...
	static AString GenerateMsgProtocolTSCode(AutoNice proList, AutoNice proNotes, Array<AString> &structNameList, bool bFirst)
	{
		...
		for (int i = 0; i < structNameList.size(); ++i)
		{
			...
			for (auto m = d->begin(); m; ++m)
			{
				...
				case FIELD_DATA:
				{
					szType.Format(": egret.ByteArray = new egret.ByteArray()");
					break;
				}
				}
				if (szType == "")
	...
	// 导出C# 消息代码
	static AString GenerateMsgProtocolC4Code(AutoNice proList, AutoNice proNotes, Array<AString> &structNameList, EasyHash<AString, bool> &exportRunHash, bool bFirst, bool bRunConfig, bool bIncludeMsgStruct)
	{
		AString cppCode = "//Auto genereate msg data code		";
		//cppCode += TimeManager::GetMe().GetDateTime();
		cppCode += "\r\n"
	...
	static AutoNice GenerateProtocol(const AString &fileName, const AString tsPath, const AString cppPath, const AString &targetName, bool bTSCode, AString error, bool bExportComment = false, bool bIncludeMsgStruct = false, AString clientMsgPath = "")
	{
