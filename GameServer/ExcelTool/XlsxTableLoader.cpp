...
AutoTable XlsxTableLoader::LoadXlsx(const AString &xlsxFile, const AString &sheetName)
{
	...
	if (sheetName == "RuleConfig")
	{
		int b = 2;
		int x = b;
		sheet = AString::getANIS(sheetName.c_str());
	}
		...
		while (!recset.IsEOF())
		{
			...
			{								
				...
				if (strItemData.GetLength() >= 253)
				{
					...
					//LOG("WWW length > %s", (LPCSTR)strItemData);

					//AString utf82 = AString::getUTF8(z2.c_str());			
					//{
					//	AString anis = AString::getANIS(utf82.c_str());

					//	FileDataStream f2("e:/Test2.txt", FILE_CREATE);
					//	f2._write((void*)anis.c_str(), anis.length());
					//	f2._write("\r\n", 2);

					//	AString info = "ANIS L : ";
					//	info += (int)anis.length();
					//	f2._write((void*)info.c_str(), info.length());

					//	f2._write("\r\n", 2);
					//	f2._write("\r\n", 2);

					//	f2._write((void*)utf82.c_str(), utf82.length());
					//	f2._write("\r\n", 2);
					//	f2._write("\r\n", 2);
					//	info = "UTF8 L : ";
					//	info += (int)anis.length();
					...
					//	f2._write("\r\n", 2);
					//	f2._write("\r\n", 2);
					//	f2._write((void*)(LPCSTR)strItemData, strItemData.GetLength());
					//	f2._write("\r\n", 2);
					//	f2._write("\r\n", 2);

					//	info = "SCR L : ";
					//	info += (int)strItemData.GetLength();
					//	f2._write((void*)info.c_str(), info.length());
					//}
					//int x = 2;
					//AString b = utf82;
