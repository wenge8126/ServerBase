...
int main()
{
	...
	logErrorFileName.Format("./Error/%s_Error_%4d-%02d-%02d_%02d-%02d-%02d.log", GameThread::GetAppName(),
		nowtime.wYear, nowtime.wMonth, nowtime.wDay, nowtime.wHour, nowtime.wMinute, nowtime.wSecond);

	Allot::setLogFile(GameThread::GetAppName());
	...
	GameThread *gpThread = MEM_NEW GameThread();
	mServer->Start(gpThread, GameThread::GetAppName(), logFileName.c_str(), NULL);
