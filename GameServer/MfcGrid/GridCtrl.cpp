...
// AFXAPI
void  _Export DDX_GridControl(CDataExchange* pDX, int nIDC, CGridCtrl& rControl)
{
...
UINT _Export GetMouseScrollLines()
{
                            ...
                            int nColumn  /* = -1 */)
{
    ...
    {
		int nP = 0;
        m_arColWidths.InsertAt(nColumn, nP);
        for (int row = 0; row < m_nRows; row++) 
        {
            GRID_ROW* pRow = m_RowData[row];
            if (!pRow) return -1;
...
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
    ...
    {
		int nP = 0;
        m_arRowHeights.InsertAt(nRow, nP);
